...
3.12. Check result and metadata to make sure everything is set as required to run SPI
...
![Panoply Open Data](https://media-exp1.licdn.com/dms/image/C5612AQFNgfvHpdk7Vg/article-inline_image-shrink_1500_2232/0?e=1607558400&v=beta&t=1ttDn50_PEnz5g9LzLJbXMFsrg-PVddvnGc-oCZUdAY)

6.2. From the Datasets tab select spi_gamma_1_month and click Create Plot

![Create Plot](https://media-exp1.licdn.com/dms/image/C5612AQGKhuiLonK_Yw/article-inline_image-shrink_1500_2232/0?e=1607558400&v=beta&t=7u6wkBziju8WHNkxg5wEHtpJ1FpKyTm2AkdperrqaXs)
...
To convert the result into GeoTIFF format, you need additional software: [GDAL](https://gdal.org) and [NCO](http://nco.sourceforge.net). 
Both software can be installed via ```Homebrew``` or ```conda```.
...
7.3. CDO required the variable should be in ```"time, lat, lon"```, while the output from SPI: ```CHIRPS_XX_spi_xxxxx_x_month.nc``` in ```"lat, lon, time"```, you can check this via ```ncdump -h file.nc```
